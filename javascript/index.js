const baseURL = "http://onesight.com.br:2052/switch/games";
...
async function filterDeveloper(developers) {
  ...
  const filteredDeveloperList = devList.filter((dev) =>
    dev.developers.includes(developers)
  );
...
async function filterPublisher(publishers) {
  ...
  const filteredPublisherList = publishList.filter((publish) =>
    publish.publishers.includes(publishers)
  );
